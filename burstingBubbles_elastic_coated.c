...
double Ohbulk, muR_cb, muR_ab, rho_cb, rho_ab, Ec, tmax, Ldomain;
int MAXlevel;
...
int main(int argc, char const *argv[]) {
  ...
  rho1 = rho_cb; mu1 = muR_cb*Ohbulk; G1 = Ec;
  rho2 = 1e0; mu2 = Ohbulk; G2 = 0.;
  rho3 = rho_ab; mu3 = muR_ab*Ohbulk; G3 = 0.;

  f1.sigma = 27.06/72; // surface tension coating-air!!!
  f2.sigma = 32.86/72; // oil and air

  fprintf(ferr, "Level %d tmax %g. Oh %3.2e, muR_cb %3.2e, rho_cb %3.2e, Ec %3.2e\n", MAXlevel, tmax, Ohbulk, muR_cb, rho_cb, Ec);
  run();
...
event logWriting (i++) {
  ...
  if (i == 0) {
    ...
    fp = fopen ("log", "w");
    fprintf(fp, "Level %d tmax %g. Oh %3.2e, muR_cb %3.2e, rho_cb %3.2e, Ec %3.2e\n", MAXlevel, tmax, Ohbulk, muR_cb, rho_cb, Ec);
